  ...
  for (const id of allIds) {
    ...
    if (relayDraft && localDraft) {
      ...
      const localHasStatusChange =
        (localDraft.status === 'rejected' || localDraft.status === 'published') &&
      ...
      if (localHasStatusChange) {
        ...
        console.log('[mergeDrafts] Keeping local status change for draft:', id, localDraft.status)
        ...
        // Relay is newer
        console.log('[mergeDrafts] Using relay version for draft:', id, 'local:', localDraft.updatedAt, 'relay:', relayDraft.updatedAt)
...
interface DraftStore {
  ...
  markAsSubmitted: (id: string, adminNpub: string, submissionId: string) => void
...
export const useDraftStore = create<DraftStore>((set, get) => ({
  ...
  loadDrafts: async () => {
    ...
    if (hasCachedDrafts) {
      console.log('[DraftStore] Loaded', cachedDrafts.length, 'drafts from cache')
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(`[DraftStore] Attempt ${attempt}/${maxRetries} failed:`, error)
    ...
    set({
      isLoading: false,
      isSyncing: false,
      error: hasCachedDrafts ? null : (lastError?.message || 'Failed to load drafts'),
    })
    isLoadingInProgress = false
  },
  ...
  markAsSubmitted: (id, adminNpub, submissionId) => {
    set((state) => {
      const newDrafts = state.drafts.map((draft) =>
        draft.id === id
          ? { ...draft, status: 'submitted' as const, submittedTo: adminNpub, lastSubmissionId: submissionId, rejectionReason: undefined, updatedAt: Date.now() }
          : draft
      )
  ...
  markAsPublished: (id, eventId) => {
    set((state) => {
      const newDrafts = state.drafts.map((draft) =>
        draft.id === id
          ? { ...draft, status: 'published' as const, publishedEventId: eventId, rejectionReason: undefined, updatedAt: Date.now() }
          : draft
      )
      saveDraftsToCache(newDrafts)
      return { drafts: newDrafts }
  ...
  markAsRejected: (id, reason) => {
    console.log('[DraftStore] markAsRejected called for draft:', id, 'reason:', reason)
    set((state) => {
      const newDrafts = state.drafts.map((draft) =>
        draft.id === id
