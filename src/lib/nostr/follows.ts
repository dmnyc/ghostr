...
export async function followUser(pubkeyToFollow: string): Promise<boolean> {
  ...
  if (!ndk || !signer || !user) {
    console.error('[Follows] Missing required services:', { ndk: !!ndk, signer: !!signer, user: !!user })
  ...
  try {
    ...
    contactListEvent.tags = Array.from(currentFollows).map(pubkey => ['p', pubkey])
    ...
    console.log('[Follows] Successfully followed user:', pubkeyToFollow.slice(0, 8))
    return true
  } catch (error) {
...
export async function unfollowUser(pubkeyToUnfollow: string): Promise<boolean> {
  ...
  try {
    ...
    console.log('[Follows] Successfully unfollowed user:', pubkeyToUnfollow.slice(0, 8))
    return true
  } catch (error) {
