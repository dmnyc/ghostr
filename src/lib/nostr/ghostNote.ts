  ...
  const timeoutPromise = new Promise<never>((_, reject) =>
    setTimeout(
      () =>
        reject(
          new Error(
            "Encryption timed out. Your signer may not support the required encryption method.",
          ),
        ),
      15000,
    ),
...
async function decryptGhostNote(
  ...
  const timeoutPromise = new Promise<never>((_, reject) =>
    setTimeout(
      () =>
        reject(
          new Error(
            "Decryption timed out. Your signer may not support the required encryption method.",
          ),
        ),
      15000,
    ),
  ...
  return await Promise.race([decryptPromise, timeoutPromise]);
}
...
export async function fetchReceivedGhostNotes(
  ...
  try {
    ...
    if (!events) {
      console.warn("[GhostNote] Fetch timed out after 10s");
      return [];
    }
...
export async function fetchGhostNoteByDTag(
  ...
  try {
    const events = await fetchWithTimeout(ndk.fetchEvents(filter), 10000);
    if (!events) {
      console.warn("[GhostNote] Fetch by d-tag timed out after 10s");
      return null;
    }
